import React from "react";

export const UploadPanel: React.FC<{
  onFile: (file: File) => void;
  detectedExchange: string | null;
  confidence: number;
  onClearAllImported: () => void;
}> = ({ onFile, detectedExchange, confidence, onClearAllImported }) => (
  <div>
    <input type="file" accept=".csv,text/csv" onChange={(e) => { const f = e.target.files?.[0]; if (f) onFile(f); }} />
    <div>Detected: {detectedExchange || "-"} ({confidence.toFixed(2)})</div>
    <button type="button" onClick={onClearAllImported}>Clear All Imported Data</button>
  </div>
);
